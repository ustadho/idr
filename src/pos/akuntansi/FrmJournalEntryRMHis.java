 ...
 * @author cak-ust
 */
public class FrmJournalEntryRMHis extends javax.swing.JInternalFrame {
    ...
    private void udfInitForm(){
        ...
        menuAuth= new MenuAuthDao(conn).getMenuByUsername("Histori Jurnal Umum", MainForm.sUserName);
        if(menuAuth!=null){
    ...
    public void udfLoadData(String arNo){
        String sQry="select distinct j.journal_no, j.tanggal, coalesce(j.description,'') as keterangan, coalesce(j.total,0) as amount\n" +
                    "from acc_journal j\n" +
                    "inner join acc_journal_detail d on d.journal_no=j.journal_no  \n" +
                    "inner join acc_coa coa on coa.acc_no=d.acc_no\n" +
                    "where not j.is_koreksi "
                    + "and j.tipe='GL' "
                    + "and j.journal_no||coalesce(j.description,'')||coalesce(d.acc_no,'')||coalesce(coa.acc_name,'') ilike '%%'\n" +
                    "and to_char(j.tanggal, 'yyyy-MM-dd')>='"+GeneralFunction.yyyymmdd_format.format(jXDatePicker1.getDate())+"'\n" +
                    "and to_char(j.tanggal, 'yyyy-MM-dd')<='"+GeneralFunction.yyyymmdd_format.format(jXDatePicker2.getDate())+"'\n" +
                    "order by j.tanggal, j.journal_no";
        ...
        try{
            ...
            while(rs.next()){
                ...
                if(arNo.equalsIgnoreCase(rs.getString("journal_no"))){
    ...
    private void udfEdit(){
        ...
        if(iRow<0)
            return;
        String arNo=tblHeader.getValueAt(iRow, tblHeader.getColumnModel().getColumnIndex("No. Jurnal")).toString();
        FrmJournalEntryRM f1=new FrmJournalEntryRM();
        f1.setConn(conn);
        f1.setTitle("Jurnal Umum - Edit");
        MainForm.jDesktopImage2.add(f1, JLayeredPane.DEFAULT_LAYER);
        f1.setJournalNo(arNo);
        f1.setSrcForm(aThis);
        f1.setVisible(true);
    }
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Histori Pembayaran Customer");
        ...
        tblHeader.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No. Jurnal", "Tanggal", "Keterangan", "Total"
            }
        ) {
...
//            param.put("SUBREPORT_DIR", sSubRpt);
    ...
    private void udfDelete() {
        ...
        if(JOptionPane.showConfirmDialog(aThis, "Anda yakin untuk menghapus bukti jurnal ini?", "Hapus data", JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
            String arNo=tblHeader.getValueAt(iRow, tblHeader.getColumnModel().getColumnIndex("No. Jurnal")).toString();
            try {
                int i=conn.createStatement().executeUpdate(
                          "delete from acc_journal where journal_no='"+arNo+"';");
                if(i>0){
