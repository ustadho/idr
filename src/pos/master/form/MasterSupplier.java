...
public class MasterSupplier extends javax.swing.JDialog {
    ...
    private RelasiDao relasoDao;
    private RelasiKategoriDao daoSupplierKat;
    ...
    private void udfInitForm(){
        try {
            ...
            cmbTipeSupp.removeAllItems();
            daoSupplierKat=new RelasiKategoriDao(conn);
            suppKategory=daoSupplierKat.cariSemuaKategori();
            for(RelasiKategori kt : suppKategory){
                cmbTipeSupp.addItem(kt.getNamaKategori());
            }
    ...
    private boolean udfCekBeforeSave(){
        if(txtNama.getText().length()==0){
            JOptionPane.showConfirmDialog(this, "Nama Supplier harus diisi!");
            txtNama.requestFocus();
            return false;
        }
        if(cmbTipeSupp.getSelectedIndex()<0){
            JOptionPane.showConfirmDialog(this, "Tipe Supplier harus dipilih!");
            cmbTipeSupp.requestFocus();
            return false;
        }
    ...
    private void udfSave() {
        try {
            ...
            Integer idTipeSupp=cmbTipeSupp.getSelectedIndex()<0? null: suppKategory.get(cmbTipeSupp.getSelectedIndex()).getId();
            if(id==null){
                txtKode.setText(relasoDao.getKode());
            }
            Relasi supp=new Relasi();
            supp.setIdRelasi(id);
            supp.setTipeRelasi(1);
            supp.setNamaRelasi(txtNama.getText());
            supp.setAlamat(txtAlamat.getText());
            supp.setIdKota(idKota);
            ...
            supp.setAccNo(cmbAkun.getSelectedIndex()>=0? coa.get(cmbAkun.getSelectedIndex()).getAcc_no(): null);
            supp.setTop(GeneralFunction.udfGetInt(txtTop.getText()));
            supp.setNoNpwp(txtNoNpwp.getText());
            supp.setActive(chkActive.isSelected());
            supp.setKode(txtKode.getText());
            supp.setIdKategori(suppKategory.get(cmbTipeSupp.getSelectedIndex()).getId());
            supp.setCust(false);
            supp.setSupp(true);
//            supp.setActive(chkActive.isSelected());
            
            relasoDao.simpan(supp);
            if(srcForm instanceof FrmListSupplier){
                ((FrmListSupplier)srcForm).tampilkanData(supp.getIdRelasi(), "");
            }
            JOptionPane.showMessageDialog(this, "Simpan supplier sukses!");
            this.dispose();
...
//        }
    ...
    private void initComponents() {
        ...
        cmbTipeSupp = new javax.swing.JComboBox();
        ...
        jPanel2.add(btnNew, new org.netbeans.lib.awtextra.AbsoluteConstraints(110, 270, 100, -1));
        ...
        jPanel2.add(btnClose, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 270, 90, -1));
        ...
        jPanel2.add(btnSave, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 270, 120, -1));
        ...
        jLabel15.setText("Tipe Supplier :");
        jPanel2.add(jLabel15, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 185, 90, 20));

        cmbTipeSupp.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        jPanel2.add(cmbTipeSupp, new org.netbeans.lib.awtextra.AbsoluteConstraints(110, 185, 170, -1));
        ...
        jPanel2.add(jLabel18, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 10, 70, 20));
        ...
        jPanel2.add(txtKode, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 10, 110, 20));
    ...
    private javax.swing.JComboBox cmbTipeSupp;
    ...
    public void tampilkanSupplier(Integer id) {
        try {
            Relasi view=relasoDao.cariByID(id);
            ...
            cmbKota.setSelectedItem(daoKota.cariByID(view.getIdKota()).getNamaKota());
            cmbTipeSupp.setSelectedItem(new RelasiKategoriDao(conn).cariKategoriByID(view.getIdKategori()).getNamaKategori());
