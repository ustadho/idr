 ...
 * @author cak-ust
 */
public class FrmPOHis extends javax.swing.JInternalFrame {
    ...
    private void udfInitForm(){
        ...
        menuAuth= new MenuAuthDao(conn).getMenuByUsername("Daftar PO", MainForm.sUserName);
    ...
    private void udfLoadItemDetail(){
        ...
        String sQry="select d.id_barang, coalesce(i.plu,'') as plu, i.nama_barang, d.qty, coalesce(i.satuan,'') as satuan, "
                ...
                + "where d.no_po='"+tblHeader.getValueAt(iRow, tblHeader.getColumnModel().getColumnIndex("No. PO")).toString() +"' "
                + "order by d.seq";
        try{
            ((DefaultTableModel)tblDetail.getModel()).setNumRows(0);
    ...
    public void udfLoadData(String sNoPo){
        String sQry="select no_po, tanggal, nama_supplier, jt_tempo, sub_total, po_disc, biaya_lain, "
                + "fn_get_Disc_Bertingkat(sub_total, po_disc)+biaya_lain as nett, description "
                + "from("
                + "	select h.no_po, h.tanggal as tanggal, coalesce(s.nama_relasi,'') as nama_supplier, h.tanggal+coalesce(h.top,0) as jt_tempo, "
                + "	sum(fn_get_Disc_Bertingkat(coalesce(d.qty,0)*coalesce(d.unit_price,0), coalesce(d.disc,''))*(1+coalesce(d.ppn,0)/100)) as sub_total, "
                + "	coalesce(h.po_disc,'') as po_disc , coalesce(h.biaya_lain,0) as biaya_lain, "
                + "     coalesce(h.description,'') as description "
                + "	from po h "
                + "	inner join po_det d on d.no_po=h.no_po "
                ...
                + "	group by h.no_po, h.tanggal, coalesce(s.nama_relasi,''), "
                + "     h.tanggal+coalesce(h.top,0), coalesce(h.po_disc,''), coalesce(h.biaya_lain,0), "
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Histori Pesanan Pembelian");
        ...
        jPanel1.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(770, 20, 130, -1));
        ...
        tblHeader.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No. PO", "Tanggal", "Supplier", "Jatuh Tempo", "Sub Total", "Disc", "Biaya Lain", "Nett", "Keterangan"
            }
        ) {
        ...
        setSize(new java.awt.Dimension(940, 549));
    ...
    public void udfRefreshDataPerBaris(String noPo) {
        String sQry="select no_po, tanggal, nama_supplier, jt_tempo, sub_total, po_disc, biaya_lain, "
                + "fn_get_Disc_Bertingkat(sub_total, po_disc)+biaya_lain as nett, description "
                + "from("
                + "	select h.no_po, h.tanggal, coalesce(s.nama_relasi,'') as nama_supplier, h.tanggal+coalesce(h.top,0) as jt_tempo, "
                + "	sum(fn_get_Disc_Bertingkat(coalesce(d.qty,0)*coalesce(d.unit_price,0), coalesce(d.disc,''))*(1+coalesce(d.ppn,0)/100)) as sub_total, "
                + "	coalesce(h.po_disc,'') as po_disc , coalesce(h.biaya_lain,0) as biaya_lain, "
                + "     coalesce(h.description,'') as description "
                + "	from po h "
                + "	inner join po_det d on d.no_po=h.no_po "
                + "	left join m_reasi s on s.id_supplier=h.supplier_id "
                + "	where h.no_po='"+noPo+"' "
                + "	group by h.no_po, h.tanggal, coalesce(s.nama_relasi,''), "
                + "     h.tanggal+coalesce(h.top,0), coalesce(h.po_disc,''), coalesce(h.biaya_lain,0), "
                + "     coalesce(h.description,'') "
        ...
        try{
            ...
            if(rs.next()){
                ...
                tblHeader.setValueAt(rs.getString("po_disc"), iRow, col.getColumnIndex("Disc"));
