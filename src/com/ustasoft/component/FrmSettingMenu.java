...
//        });
    ...
    private void udfLoadMenuUserAuth() {
        ...
        String sQry="select list.id, menu_description ,coalesce(can_insert, false) as can_insert, coalesce(can_update, false) as can_update, " +
                ...
                (cmbGroup.getSelectedIndex()==0?  "" : "inner join m_menu_list_grouping g on g.menu_id=list.id "
                + "and g.group_id='"+cmbGroup.getSelectedItem().toString()+"' ") +
...
//            }
    ...
    private void udfInitForm(){
        try{
            ResultSet rs=conn.createStatement().executeQuery("select user_name from m_user " +
                    "order by upper(user_name)");
            DefaultListModel lstModel=new DefaultListModel();

            while(rs.next()){
                lstModel.addElement(rs.getString("user_name"));
            }
